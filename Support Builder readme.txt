...
The reason the support skill information exists in .txt form and not in the source code is that it is designed to be more fluid with the release of new support abilities. Instead of needing to edit the source code with each release one simply edits the correpsonding .txt file(s). This also allows
...
The following text gives the templates for each file you will find in the "Data" directory.
	...
	*** numericalTypes.txt ***
		...
		Choosing another entry (from line 271):
		
			4	15	4	1114241	1
			
		Looking at line 271 in "characterNames.txt", we see that these values belong to "Jango Fett".
...
set of affiliations.
	...
	V.i Support Decks Output
		...
		written to the files called "Hp_5_Null_Prep_5.txt" and "Hp_5_Null_Prep_5_EMAX.txt". If no combinations can be found for a particular
		search, the created output files are deleted.
	...
	V.ii Unique Profile Output
		...
		the game. There are two possible files that can be written into that folder:
